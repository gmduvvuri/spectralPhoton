...
def _FITSformat(dtype):
    ...
    if dstr in ["float64", "f8"]:
        return "D"
    raise ValueError(
        "Not sure waht to do with the {} data type.".format(dstr)
    )
...
class Photons:
    """
    Base class for holding spectral photon data. Each instance contains a list
    of observed photons, including,
    at a minimum:
    - time of arrival, t
    - wavelength, w
    ...
    - region weight (1.0 for signal,
                     -signal_area/backgroun_area for background,
                     0.0 otherwise), r
    ...
    These attributes are all protected in this class and
    are accessed using the bracket syntax, e.g. if `photons` is an
    ...
    I thought about deriving this class from FITS_rec or _np.recarray,
    but ultimately it felt cumbersome. I decided
    it's better they be a part of the object, rather than
    a parent to it, since it will also contain various metadata.
    """

    # for ease of use, map some alternative names
    # to the proper photon property names
    _alternate_names = {
        "time": "t",
        "wavelength": "w",
        "wave": "w",
    ...
    def __init__(self, **kwargs):
        """
        Create an empty Photons object. This is really just
        to show what the derived classes must define. If I ever
        see a need to be able to create Photons objects using
        arrays of data in the future, then I'll rewrite this.

        Parameters
        ----------
        obs_metadata : list of metadata dict (or similar types)
                       for each obs
        time_datum : time object
        data : astropy Table with at least a 't' and 'w' column
               (see class description for what to name others
        ...
        # metadata associated with the observations that recorded the photons,
        # one list entry for each observation
        self.obs_metadata = kwargs.get("obs_metadata", [{}])

        self.time_datum = kwargs.get(
            "time_datum", _time.Time("2000-01-01T00:00:00")
        )

        if "photons" in kwargs:
            self.photons = kwargs["photons"]
        else:
            self.photons = _tbl.Table(
                names=["t", "w"], dtype=["f8", "f8"]
            )
        ...
        if "obs_times" in kwargs:
            ...
            if "n" in self and len(self) > 0:
                ...
                rngs = [
                    _np.array([[a["t"].min(), a["t"].max()]])
                    for a in self.photons.groups
                ]
                self.obs_times = rngs
            elif len(self) > 0:
                self.obs_times = [
    ...
    _ovr_doc = (
        '"overlap_handling : "adjust Aeff"|"clip"\n'
    ...
    def merge_like_observations(
        self, overlap_handling="adjust Aeff", min_rate_ratio=0.5
    ):
        """
        ...
        while i < len(self.obs_metadata):
            ...
            while j < len(self.obs_metadata):
                ...
                if _np.all(self.obs_times[i] == self.obs_times[j]):
                    i_photons = self["n"] == i
                    j_photons = self["n"] == j
                    overlap = utils.rangeset_intersect(
                        self.obs_bandpasses[i], self.obs_bandpasses[j]
                    )
                    if len(overlap) > 0:
                        ...
                        worthwhile = (
                            (Ni > 0)
                            and (Nj > 0)
                            and (Ni / Nj > min_rate_ratio)
                            and (Nj / Ni > min_rate_ratio)
                        )
                        if (
                            overlap_handling == "adjust Aeff"
                            and worthwhile
                        ):
                            ...
                            overlap_handling == "clip" or not worthwhile
                        ):
                            if Ni < Nj:
                                (ii,) = _np.nonzero(xi)
                                ...
                                self.obs_bandpasses[
                                    i
                                ] = utils.rangeset_subtract(
                                    self.obs_bandpasses[i], overlap
                                )
                            else:
                                (jj,) = _np.nonzero(xj)
                                ...
                                self.obs_bandpasses[
                                    ...
                                    self.obs_bandpasses[j], overlap
                                )
                            j_photons = self["n"] == j
                            signal = get_signal()
                        else:
                            raise ValueError(
                                "overlap_handling option not recognized."
                            )
                    ...
                    self.obs_bandpasses[i] = utils.rangeset_union(
                        self.obs_bandpasses[i], self.obs_bandpasses[j]
                    )
    ...
    merge_like_observations.__doc__ = (
        merge_like_observations.__doc__.format(ovr=_ovr_doc)
    )
    ...
    def merge_orders(self, overlap_handling="adjust Aeff"):
        ...
        if len(self.obs_times) > 1:
            separate = [
                self.get_obs(i) for i in range(len(self.obs_times))
            ]
        else:
            separate = [self]
        for obj in separate:
            ...
            obj.obs_bandpasses = [
                order_ranges[[i]] for i in range(len(order_ranges))
            ]
            obj.photons["n"] = obj.photons["o"] - _np.min(
                obj.photons["o"]
            )
            obj.photons.remove_column("o")
            obj.merge_like_observations(
    ...
    def __add__(self, other):
        ...
        if not isinstance(other, Photons):
            raise ValueError(
                "Can only add a Photons object to another Photons object."
            )
        ...
        obs_bandpasses = list(self.obs_bandpasses) + list(
            other.obs_bandpasses
        )

        return Photons(
            photons=photons,
            obs_metadata=obs_metadata,
            time_datum=self.time_datum,
            obs_times=obs_times,
    ...
    def copy(self):
        ...
        new.obs_bandpasses = [
            item.copy() for item in self.obs_bandpasses
        ]
        return new
    ...
    def writeFITS(self, path, overwrite=False):
        ...
        for colname in self.photons.colnames:
            ...
            fits_col = _fits.Column(
                name=name,
                format=format,
                array=tbl_col.data,
                unit=str(tbl_col.unit),
            )
            photon_cols.append(fits_col)
        photon_hdr = _fits.Header()
        photon_hdr["zerotime"] = (
            self.time_datum.jd,
            "julian date of time = 0.0",
        )
        photon_hdu = _fits.BinTableHDU.from_columns(
            photon_cols, header=photon_hdr
        )

        # save obs metadata, time ranges, bandpasses to additional extensions
        obs_hdus = []
        for meta, times, bands in zip(
            self.obs_metadata, self.obs_times, self.obs_bandpasses
        ):
            ...
            if isinstance(meta, _fits.Header):
                ...
                raise ValueError(
                    "FITS file cannot be constructed because Photons object has an improper list of "
            ...
            units = [str(self["w"].unit)] * 2 + [
                str(self["t"].unit)
            ] * 2
            formats = ["{}D".format(len(a)) for a in arys]
            info_cols = [
                _fits.Column(
                    array=a.reshape([1, -1]), name=n, unit=u, format=fmt
                )
    ...
    def loadFITS(cls, path):
        ...
        def parse_bands_and_time(extension):
            bandpas0, bandpas1, start, stop = [
                extension.data[s]
                for s in ["bandpas0", "bandpas1", "start", "stop"]
            ]
    ...
    def set_time_datum(self, new_datum=None):
        ...
        if new_datum is None:
            dt = _time.TimeDelta(
                self["t"].min(), format=self["t"].unit.to_string()
            )
        ...
        need_bytes = _np.ceil(need_bits / 8.0)
        if need_bytes > 8:
            raise ValueError(
                "Resetting the time atum of this observation by {} {} will result in loss of numerical "
                "precision of the photon arrival times.".format(
                    dt, self["t"].unit
                )
            )
        ...
        for key in self.photons.colnames:
            if key in other.photons.colnames:
                if other[key].unit:
                    ...
                    if (
                        str(unit).lower() == "none"
                        and str(self[key].unit).lower() == "none"
                    ):
                        continue
    ...
    def add_observations_column(self):
        ...
        if "n" not in self:
            if len(self.obs_metadata) > 1:
                raise ValueError(
                    "Photons cannot be assigned to multiple observations because who the F knows which "
    ...
    def divvy(self, ysignal, yback=(), order=None):
        ...
        # groom the input
        ysignal, yback = [
            _np.reshape(a, [-1, 2]) for a in [ysignal, yback]
        ]
        assert order is None or type(order) == int

        # join the edges into one list
        edges, isignal, iback, area_ratio = self._get_ribbon_edges(
            ysignal, yback
        )
        ...
        signal[filter] = reduce(
            _np.logical_or, [ii == i for i in isignal]
        )
        self["r"][signal] = 1.0
        if len(yback) > 0:
            bkgnd = _np.zeros(len(self), bool)
            bkgnd[filter] = reduce(
                _np.logical_or, [ii == i for i in iback]
            )
    ...
    def squish(self, keep="both"):
        ...
        if "r" not in self:
            raise ValueError(
                "Photon object must have an 'r' column (specifying a region weight) before it can be "
                "squished."
            )
        valid_keeps = (
            ["both"] + ["back", "background", "bg"] + ["signal"]
        )
        if keep not in valid_keeps:
            raise ValueError(
                "keep parameter must be one of {}".format(valid_keeps)
            )
    ...
    def spectrum(
        ...
        """
        filter = self._filter_boiler(
            waveranges=None, time_ranges=time_ranges, order=order
        )

        bin_edges, i_gaps = self._groom_wbins(
            bins, waveranges, bin_method=bin_method
        )
        counts, errors = self._histogram(
            "w",
            bin_edges,
            None,
            fluxed,
        ...
        bin_exptime = self.time_per_bin(
            bin_edges, time_ranges=time_ranges
        )
        bin_exptime[
            bin_exptime == 0
        ] = _np.nan  # use nans to avoid division errors
    ...
    def spectrum_smooth(
        ...
        filter = self._filter_boiler(
            waveranges=wave_range, time_ranges=time_ranges, order=order
        )
        ...
        (
            ...
            rates,
            errors,
        ) = _smooth_boilerplate(w, weights, n, wave_range)

        # divide by time to get rates
        bin_exptimes = self.time_per_bin(
            [bin_start, bin_stop], time_ranges
        )
    ...
    def lightcurve(
        ...
        if not _np.any(covered):
            raise ValueError(
                "None of the observations cover the provided bandpasses."
            )
        ...
        if hasattr(time_step, "__iter__"):
            ...
            valid_times = utils.rangeset_intersect(
                _np.vstack(self.obs_times), _np.array(time_step)
            )
            valid_bins = utils.inranges(mids, valid_times)
        else:
            edges, valid_bins = self._construct_time_bins(
                time_step, bin_method, time_range, bandpasses
            )

        inbands = self._bandpass_filter(
            bandpasses, check_coverage=False
        )
        filter = self._filter_boiler(
            waveranges=None, time_ranges=None, order=order
        )
        ...
        if nan_between:
            ...
            counts, errors, dt, bin_start, bin_stop = [
                a[valid_bins]
                for a in [counts, errors, dt, bin_start, bin_stop]
            ]
    ...
    def lightcurve_smooth(
        ...
        filter = self._filter_boiler(
            waveranges=None, time_ranges=time_range, order=order
        )
        ...
        curves = (
            []
        )  # each curve in list will have bin_start, bin_stop, bin_midpts, rates, error
        ...
        if len(curves) > 0:
            bin_start, bin_stop, bin_midpt, rates, error = [
                _np.hstack(a) for a in zip(*curves)
            ]
        else:
            bin_start, bin_stop, bin_midpt, rates, error = [
                _np.array([]) for _ in range(5)
            ]
    ...
    def spectrum_frames(self, bins, time_step, **kws):
        ...
        if progress_bar:
            try:
                ...
                raise ImportError(
                    "You need the tqdm module for a progress " "bar."
                )
        else:
            bar = lambda x: x
        kws = dict(
            fluxed=fluxed, energy_units=energy_units, order=order
        )

        # make wbins ahead of time so this doesn't happen in loop
        bin_edges, i_gaps = self._groom_wbins(
            bins, waveranges, bin_method=w_bin_method
        )
        kws["bins"] = bin_edges
        ...
        if hasattr(time_step, "__iter__"):
            print(
                "I see that you gave user supplied time bins. That is fine, but note that no checks will be "
            ...
            time_edges, valid_time_bins = self._construct_time_bins(
                time_step, t_bin_method, time_range
            )
            starts, stops = (
                time_edges[:-1][valid_time_bins],
                time_edges[1:][valid_time_bins],
            )
        ...
        bin_edges, bin_midpts, rates, errors = list(
            map(_np.array, list(zip(*spectra)))
        )
    ...
    def continuum_subtracted_lightcurves(
        self,
        dt,
        dw,
        continuum_bands,
        ...
        energy_units="erg",
        time_range=None,
        w_bin_method="elastic",
        t_bin_method="elastic",
        progress_bar=False,
    ):
        kws = dict(fluxed=fluxed, energy_units=energy_units)

        t0, t1, t, we, w, f, e = self.spectrum_frames(
            dw,
            dt,
            waveranges=continuum_bands,
            w_bin_method=w_bin_method,
            t_bin_method=t_bin_method,
        ...
        for i in range(len(t)):
            _, _, pf = utils.polyfit_binned(
                wbins, f[i, good] * dw, e[i, good] * dw, poly_order
            )
            polyfuncs.append(pf)
        ...
        for band in lc_bands:
            _, _, _, tf, te = self.lightcurve(
                tbins, band, bin_method=t_bin_method, **kws
            )
    ...
    def which_obs(self, t):
        ...
        for n, tranges in enumerate(self.obs_times):
            ...
            if not _np.all(obsno[inobs] == -1):
                raise ValueError(
                    "Some observation time ranges overlap at the input times, so these times cannot be "
    ...
    def image(
        self, xax, yax, bins, weighted=False, scalefunc=None, show=True
    ):
        """
        ...
        counts, xbins, ybins = _np.histogram2d(
            self[xax], self[yax], bins=bins, weights=weights
        )
        ...
        if show:
            ...
            plt.imshow(
                scaled_counts[:, ::-1].T, extent=extent, aspect="auto"
            )
    ...
    def time_per_bin(self, bin_edges, time_ranges=None):
        ...
        for tranges, wranges in zip(
            self.obs_times, self.obs_bandpasses
        ):
            tranges = _np.copy(tranges)
            if time_ranges is not None:
                tranges = utils.rangeset_intersect(
                    time_ranges, tranges, presorted=True
                )
            if len(tranges) == 0:
                continue
            ...
            if len(wranges) > 1:
                ...
                wranges = reduce(
                    utils.rangeset_union, wranges[1:], wranges[:1]
                )
    ...
    def clean_obs_times(self, bandpasses=None):
        ...
        if bandpasses is None:
            ...
            if not _np.any(covered):
                raise ValueError(
                    "No observations cover the provided bandpasses."
                )
            obs_times_lst = [
                self.obs_times[i] for i in _np.nonzero(covered)[0]
            ]
    ...
    def _get_proper_key(self, key):
        ...
        if key in list(self._alternate_names.values()):
            ...
            raise KeyError(
                "{} not recognized as a field name".format(key)
            )
    ...
    def _get_ribbon_edges(self, ysignal, yback):
        ...
        if any(ys[:-1, 1] > ys[1:, 0]):
            raise ValueError(
                "There is overlap in the signal and background regions. That's a no-no."
            )
        ...
        if ymax < edges.max() or ymin > edges.min():
            raise ValueError(
                "Extraction ribbons include areas beyond the range of the counts."
            )
    ...
    def _compute_epera(self, units="erg"):
        ...
        if "a" not in self:
            raise ValueError(
                "Photons must have effective area data to permit the computation of fluxes."
            )
    ...
    def _construct_time_bins(
        self, time_step, bin_method, time_range=None, bandpasses=None
    ):
        ...
        if bin_method not in validspecs:
            raise ValueError(
                "binspec must be one of {}".format(validspecs)
            )
        ...
        for rng in self.clean_obs_times(bandpasses):
            ...
            valid.extend(
                list(range(marker, marker + len(obs_bins) - 1))
            )
            marker += len(obs_bins)
    ...
    def _bandpass_filter(self, bandpasses, check_coverage=True):
        ...
        if check_coverage and not _np.all(
            self.check_wavelength_coverage(bands)
        ):
            raise ValueError(
                "Some bandpasses fall outside of the observation ranges."
            )
    ...
    def _histogram(
        ...
        if background:
            if "r" not in self:
                raise ValueError(
                    "Background region not defined for this photon set."
                )
            if fluxed:
                raise ValueError(
                    "Does not make sense to compute a fluxed background lightcurve."
                )
            weights = self["r"] < 0
        ...
        weights[_np.isnan(weights)] = 0.0

        counts = _np.histogram(
            x, bins=bin_edges, range=rng, weights=weights
        )[0]
        variances = _np.histogram(
            x, bins=bin_edges, range=rng, weights=weights**2
        )[0]
        ...
        if _np.any(counts <= 0):
            if background:
                ...
                signal = (
                    self["r"][keep] > 0
                    if "r" in self
                    else _np.ones(len(x), bool)
                )
                signal_counts = _np.histogram(
                    x[signal], bins=bin_edges, range=rng
                )[0]
                signal_counts_weighted = _np.histogram(
                    x[signal],
                    bins=bin_edges,
                    range=rng,
                ...
                if any(zeros):
                    ...
                    if _np.sum(zeros) / len(zeros) > 0.5:
                        if len(weights) == 0 or _np.all(weights <= 0):
                            all_weights = self._full_weights(
                                fluxed, energy_units
                            )
                            signal_counts_weighted = _np.nanmedian(
                        ...
                        signal_counts_weighted[zeros] = _np.interp(
                            bin_midpts[zeros],
                            bin_midpts[~zeros],
                            signal_counts_weighted[~zeros],
                        )
    ...
    def _filter_boiler(self, waveranges, time_ranges, order):
        filter = _np.ones(len(self), bool)
        if order is None:
            if "o" in self and not _np.allclose(
                self["o"][0], self["o"]
            ):
                raise ValueError(
                    "You must specify an order number when there are multiple orders in an observation.  "
...
def _groom_bins(bins, rng, bin_method="full"):
    ...
    if isinstance(bins, float):
        ...
        if (x1 - x0) % dx == 0:
            ...
            edges_full = (
                edges_left_aligned + (x1 - edges_left_aligned[-1]) / 2.0
            )
            if bin_method == "full":
                return edges_full
            if bin_method == "partial":
                return _np.insert(
                    edges_full,
                    [0, len(edges_full)],
                    [edges_full[0] - dx, edges_full[-1] + dx],
                )
    ...
    # if bins is iterable, just give it back
        ...
        raise ValueError(
            "Input bins must be either an integer, float, or iterable."
        )
    return edges
...
def _smooth_boilerplate(x, weights, n, xrange=None, independent=False):
    ...
    if len(xu) < len(x):
        edges = (xu[1:] + xu[:-1]) / 2.0
        edges = _np.insert(
            edges, [0, len(edges)], [edges[0] - 1, edges[-1] + 1]
        )
    ...
    if independent:
        ...
        bin_start, bin_stop, counts, errors = [
            a[slc] for a in [bin_start, bin_stop, counts, errors]
        ]
...
class Spectrum(object):
    ...
    def __init__(
        ...
        if wbins is None:
            if (
                type(dw) is str and dw == "guess"
            ):  # first part avoids comparing array to string
                wbins = utils.wave_edges(w.value) * w.unit
            else:
                dw = dw.to(w.unit)
                wbins = (
                    utils.edges_from_mids_diffs(w.value, dw.value)
                    * w.unit
                )
                if not _np.allclose(utils.midpts(wbins).value, w.value):
                    raise ValueError(
                        "The bin width (dw) values you provided are not consistent with the wavelength "
                        "midpoints (w)."
                    )
        self.wbins = wbins
        self.y = y
    ...
    def __getattr__(self, key):
        if key in self.ynames:
            ...
            raise AttributeError(
                "No {} data associated with this spectrum. Use add_data if you want to add it."
                "".format(key)
            )
    ...
    def _groom_integration_ranges(cls, *args):
        if len(args) == 1:
            ...
            raise ValueError(
                "Input must either be [[w0,w1],[w2,w3],...], [w0,w1], or w0, w1."
            )
        wbin = wbin.reshape(-1, 2)
        if _np.any(wbin[1:, 0] <= wbin[:-1, 1]):
            raise ValueError(
                "No overlapping or touching ranges allowed."
            )
        return wbin
    ...
    def rebin(self, newbins, other_data_bin_methods="avg"):
        ...
        if multi:
            ...
            for nb in newbins:
                specs.append(
                    self.rebin(
                        nb,
                        other_data_bin_methods=other_data_bin_methods,
                    )
                )
        ...
        if self.other_data is None:
            ...
            if type(other_data_bin_methods) is str:
                methods = [other_data_bin_methods] * len(
                    self.other_data
                )
                methods = dict(
                    list(zip(list(self.other_data.keys()), methods))
                )
            else:
            ...
            for key, d in list(self.other_data.items()):
                ...
                method = methods[key]
                other_data_new[key] = (
                    _rebin.rebin(nb, ob, d.value, method) * d.unit
                )

        meta, refs = [
            _copy.copy(a) for a in [self.meta, self.references]
        ]
        newspec = Spectrum(
            None,
            ynew,
    ...
    def integrate(self, *args):
        ...
        try:
            ...
            raise ValueError(
                "Input must be astropy quantity with units "
                "convertable to that of spectrum."
            )
        if _np.any(wbin < self.wbins[0]) or _np.any(
            wbin > self.wbins[-1]
        ):
            raise ValueError(
                "Integration range is beyond spectrum range."
            )
    ...
    def write(self, path, overwrite=False):
        ...
        tbl.write(
            path,
            format=Spectrum.table_write_format,
            overwrite=overwrite,
        )
    ...
    def read_muscles(cls, path, format=None):
        ...
        try:
            w0, w1, f, e = [
                path[s].quantity for s in ["w0", "w1", "flux", "error"]
            ]
        except KeyError:
            w0, w1, f, e = [
                path[s].quantity
                for s in ["WAVELENGTH0", "WAVELENGTH1", "FLUX", "ERROR"]
            ]
        ...
        wedges = (
            _np.unique(_np.concatenate([w0.value, w1.value])) * w0.unit
        )
    ...
    def read(cls, path_or_file_like):
        ...
        if type(
            path_or_file_like
        ) is str and not path_or_file_like.endswith(cls.file_suffix):
            raise IOError(
                "Can only read {} file.".format(cls.file_suffix)
            )
    ...
    def blackbody(cls, T, wbins):
        ...
        f = (
            ...
            * _const.c**2
            / w**5
            / (_np.exp(_const.h * _const.c / _const.k_B / T / w) - 1)
        )
        f = f.to("erg s-1 cm-2 AA-1")
        return Spectrum(
            None, f, yname=["f", "flux", "surface flux"], wbins=wbins
        )
...
class MultiSpectrum(object):
    ...
    def __setattr__(self, key, value):
        if key == "spectra":
            raise ValueError(
                "Can only set spectra attribute on " "initialization."
            )
        if hasattr(value, "__iter__") and len(value) == len(
            self.spectra
        ):
            ...
            raise ValueError(
                "Input could not be shaped to match spectra."
            )
...
class GappySpectrum(MultiSpectrum):
    ...
    def __init__(self, *args, **kws):
        ...
        blocks = _np.array(
            [spec.wbins[[0, -1]] for spec in self.spectra]
        )
        ...
        if _np.any(blocks[1:, 0] < blocks[:-1, 1]):
            raise ValueError(
                "The spectra making up a GappySpectrum should "
                "not overlap."
            )

        self.__dict__["i_gaps"] = _np.cumsum(
            list(map(len, self.spectra))
        )
    ...
    def wranges(self):
        ...
        ranges = _np.array(
            [spec.wbins[[0, -1]].to(wunit) for spec in self.spectra]
        )
        return ranges * wunit
    ...
    def any_gap_overalp(self, other_wranges, strict=False):
        ...
        gap_overlap = utils.rangeset_subtract(
            other_wranges.value, self.wranges.value
        )
        if strict:
            ...
            return _np.any(gap_lens > dwmin / 100.0)
    ...
    def integrate(self, *args, **kws):
        ...
        if self.any_gap_overalp(wbin):
            if gap_handling == "error":
                raise ValueError(
                    "Some of the integration ranges cover gaps "
                    "in the spectrum."
                )
                ...
                raise ValueError(
                    "gap_handling parameter not recognized"
                )
    ...
    def rebin(
        ...
        if self.any_gap_overalp(newbins2D):
            if gap_handling == "error":
                raise ValueError(
                    "Spectrum does not fully cover newbins."
                )
            if gap_handling == "intersect":
                newbins2D = utils.rangeset_intersect(
                    wranges.value, newbins2D.value
                )
                newbins2D = newbins2D * wunit
                newbins2D_list = utils.split_at_gaps(newbins2D)
                newbin_list = list(
                    map(utils.bins_2D_to_1D, newbins2D_list)
                )
            else:
                raise ValueError(
                    "value for gap_handling not recognized"
                )
        ...
        for bins in newbin_list:
            for spec in self.spectra:
                if (
                    spec.wbins[0] <= bins[0]
                    and spec.wbins[-1] >= bins[-1]
                ):
                    specs.append(
                        spec.rebin(bins, other_data_bin_methods=odbm)
                    )
